...
- Relative paths used by `page.content[].path`, `today-section.notesFile`, task `notesFile`, `create-survey.fromFile`, and `create-quiz` `fromFile` / `promptFile` are resolved against `apps/cli/session-assets/<course-name>/` first, then fall back to the blueprint folder for backward compatibility.
